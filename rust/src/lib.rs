// `ConsumeError<E>` carries a `tonic::Status` in its `Transport` variant, which
// puts the enum over clippy's `result_large_err` threshold (~176 bytes) at every
// `Result<_, ConsumeError<E>>` in cdc.rs. The suggested fix is to box it, but
// that reshapes a public type callers match on, so the lint is turned off here
...
pub mod proto {
    // Generated by tonic-build from proto/statelet.proto. Lints are suppressed
    // wholesale because the contents are not ours to edit: doc comments here
    // are verbatim proto comments, and their formatting trips
    // `doc_lazy_continuation` on every regeneration.
    #![allow(clippy::all)]
